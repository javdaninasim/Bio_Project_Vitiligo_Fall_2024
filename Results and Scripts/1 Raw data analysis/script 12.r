...
gsms <- "101010101010XXXXXXXXXXXXXXXXXX"
...
gs <- factor(sml)
groups <- make.names(c("lesional","ctrl"))
levels(gs) <- groups
gset$group <- gs
...
tT <- subset(tT, select=c("ID","adj.P.Val","logFC","GI","SEQUENCE","GB_ACC","Gene.symbol","Gene.ID","GI","GenBank.Accession"))

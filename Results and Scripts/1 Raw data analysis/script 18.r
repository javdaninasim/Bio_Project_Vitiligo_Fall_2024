...
gset <- getGEO("GSE75819", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
gsms <- "XXXXXXXXXXXX010101010101010101"
...
gs <- factor(sml)
groups <- make.names(c("ctrl","lesional"))
levels(gs) <- groups
gset$group <- gs
...
tT <- subset(tT, select=c("ID","adj.P.Val","logFC","Unigene_ID","Entrez_Gene_ID","Accession","Gene.symbol","Gene.ID","GI","GenBank.Accession"))
...
dT <- decideTests(fit2, adjust.method="holm", p.value=0.05, lfc=1.5)
...
par(mar=c(7,4,2,1))
title <- paste ("GSE75819", "/", annotation(gset), sep ="")
boxplot(ex[,ord], boxwex=0.6, notch=T, main=title, outline=FALSE, las=2, col=gs[ord])
legend("topleft", groups, fill=palette(), bty="n")

# expression value distribution
par(mar=c(4,4,2,1))
title <- paste ("GSE75819", "/", annotation(gset), " value distribution", sep ="")
...
plotSA(fit2, main="Mean variance trend, GSE75819")